...
namespace Evento.Test
{
    public class TestAdoEvento
    {
        ...
        public async Task Obtener_Todos_Los_Eventos_Debe_Devolver_Lista()
        {
            ...
            {
                new Eventos { idEvento = 1, Nombre = "Concierto" },
                new Eventos { idEvento = 2, Nombre = "Obra de teatro" }
            };
        ...
        {
            var mockRepo = new Mock<IRepoEvento>();
            var evento = new Eventos { idEvento = 1, Nombre = "Concierto" };
        ...
        [Fact]
        public async Task Obtener_TipoEvento_Por_Nombre_Debe_Devolver_TipoEventoDto()
        {
            var mockRepo = new Mock<IRepoEvento>();
            var tipoDto = new TipoEventoDto { idTipoEvento = 1, tipoEvento = ETipoEvento.Entretenimiento };
            mockRepo.Setup(r => r.ObtenerTipoEventoPorNombre("Entretenimiento")).ReturnsAsync(tipoDto);

            var resultado = await mockRepo.Object.ObtenerTipoEventoPorNombre("Entretenimiento");

            Assert.NotNull(resultado);
            Assert.Equal(1, resultado.idTipoEvento);
            Assert.Equal("Entretenimiento", resultado.tipoEvento.ToString());
        }
        ...
        public async Task Obtener_Funciones_Por_Evento_Debe_Devolver_Lista()
        {
            ...
            {
                new Funcion { idFuncion = 1, Nombre = "Función 1" },
                new Funcion { idFuncion = 2, Nombre = "Función 2" }
            };
            mockRepo.Setup(r => r.ObtenerFuncionesPorEvento(1)).ReturnsAsync(funciones);

            var resultado = await mockRepo.Object.ObtenerFuncionesPorEvento(1);
